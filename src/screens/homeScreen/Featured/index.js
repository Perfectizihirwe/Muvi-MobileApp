...
export function FeaturedScreen(props) {
          ...
          {trending_movies?.map((movies, index) => {
            return (
              <TrendingComponent
                key={movies.id}
                id={movies.id}
                trendTitle={movies.title || movies.name}
                trendImage={movies.backdrop_path}
                trendDate={movies.release_date || movies.first_air_date}
                navigation={() => {
          ...
          {popular_movies?.map((trendMovies, index) => {
            return (
              <PopularComponent
                ...
                navigation={() => {
                  props.navigation.navigate("DetailScreen", trendMovies);
                }}
              />
            );
          })}
          ...
          {new_movies?.map((new_movies, index) => {
            return (
              <NewMoviesComponent
                ...
                newDate={new_movies.release_date || new_movies.first_air_date}
                navigation={() => {
                  props.navigation.navigate("DetailScreen", new_movies);
                }}
              />
            );
          })}
